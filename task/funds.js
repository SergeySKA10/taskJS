...
// Если хотя быодин из объектов содержит отрицательное значение поля amount, то функция возвращает сумму всех значений. (число) Если таких значений нет - запускается функция getPositiveIncomeAmount с тем же массивом данных.
...
console.log( getPositiveIncomeAmount(funds2) );


const getTotalIncomeAmount = (data) => {
    return [...data].filter(item => item.amount > 0).length === data.length ? 
            getPositiveIncomeAmount(data) 
            :data.reduce((acc, item) => acc + item.amount, 0);
};

console.log( getTotalIncomeAmount(funds) );
console.log( getTotalIncomeAmount(funds2) );
