...
// or
// [North, South, South, East, West, North, West]
// You can immediately see that going "NORTH" and immediately "SOUTH" is not reasonable, better stay to the same place! So the task is to give to the man 
...
function dirReduc(arr){
    function iterationPath (array) {
      const path = [];
      let n = 'NORTH', s = 'SOUTH', w = 'WEST', e = 'EAST';

      loop1: for (let i = 0; i < array.length; i++) {
        if (array[i] === n && array[i+1] === s || array[i] === s && array[i+1] === n || array[i] === w && array[i+1] === e || array[i] === e && array[i+1] === w) {
          i += 1;
          continue loop1;
        } else {
          path.push(array[i]);
    ...
    const coincidences = iterationPath(arr);
    const path = iterationPath(coincidences);
