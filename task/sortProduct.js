...
const products = [
    ...
    {ratingRevievs: "403 отзыва", price: "4 349грн", name: "Xiaomi Redmi Note 4X 3/32GB Black"}, 
...
const quickSort = (arr, param) => {
    if (arr.length < 2) {
        return arr;
    }

    const support =  arr[Math.floor((arr.length - 1) / 2)][param].replace(/\D/g, '').trim();
          lesser = [],
          greater = [];
    ...
    // return [...quickSort(lesser), support, ...quickSort(greater)];
}

console.log( quickSort(products, 'ratingRevievs') )